{{-- resources/views/studio/dashboard.blade.php --}}
@extends('layouts.app')
...
<div class="page-content">
  <section class="section dashM px-2 px-md-3 pt-2 pb-1">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
      <h4 class="mb-0 fw-bold">DASHBOARD STUDIO</h4>
        ...
        <div class="kpi-card card h-100">
          <div class="kpi-body">
            ...
            <div>
              ...
              <p class="kpi-value display-6 fw-bold mb-0" id="kpiSurvei">{{ $kpi['survei'] ?? 0 }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col">
        <div class="kpi-card card h-100">
          <div class="kpi-body">
            ...
            <div>
              <div class="kpi-title">Denah & Moodboard</div>
              <p class="kpi-value display-6 fw-bold mb-0" id="kpiDenah">{{ $kpi['denah'] ?? 0 }}</p>
            </div>
          </div>
        </div>
      </div>
...
<div class="col">
  <div class="kpi-card card h-100">
    <div class="kpi-body">
      <div class="kpi-icon purple"><i class="bi bi-brush"></i></div>
      <div>
        <div class="kpi-title">3D Desain</div>
        <p class="kpi-value display-6 fw-bold mb-0" id="kpiExtInt">{{ $kpi['extint'] }}</p>
      </div>
    </div>
  </div>
</div>
...
<div class="col">
  <div class="kpi-card card h-100">
    <div class="kpi-body">
      <div class="kpi-icon orange"><i class="bi bi-gear-wide-connected"></i></div>
      <div>
        <div class="kpi-title">MEP & Spek</div>
        <p class="kpi-value display-6 fw-bold mb-0" id="kpiMep">{{ $kpi['mep'] }}</p>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="row charts-row mt-3">
      <div class="col-12 col-xl-9">
        <div class="card shadow-sm border-0 card-tight mb-3">
          <div class="card-body">
            <div class="fw-semibold mb-2">Total Klien Studio</div>
...
(() => {
  ...
  // FIX: jangan (object){}
let kelasPct = @json($kelasPct ?? (object)[]);
  ...
  const elDenah  = document.getElementById('kpiDenah');
  const elExtInt = document.getElementById('kpiExtInt');
  const elMep    = document.getElementById('kpiMep');
  ...
  async function reloadStats(q){
    ...
    const res = await fetch(`{{ route('studio.dashboard.stats') }}?`+params.toString(),{headers:{'X-Requested-With':'XMLHttpRequest'}});
    const js = await res.json();

    // KPI
    elSurvei.textContent = js.kpi.survei ?? 0;
    elCancel.textContent = js.kpi.cancel ?? 0;
    elDenah.textContent  = js.kpi.denah  ?? 0;
    elExtInt.textContent = js.kpi.extint ?? 0;
    elMep.textContent    = js.kpi.mep    ?? 0;
